        ...
        .then((data) => {
            ...
            if (!(data.items)) content.innerHTML = "<table> <thead> <tr> <th>Show Name</th> <th>Tickets Available</th> </tr></thead><tbody><tr><td colspan='2'>No shows available</td></tr></tbody></table>";
                ...
                for (var i = 0; i < data.items.length; i++) {
                    document.getElementById("tb").innerHTML += "<tr><td>" + data.items[i].name + "</td><td>" + data.items[i].available + "</td><td class='transparent'><button onclick='details(this);' class='transparent' value = " + data.items[i].key + ">Details</button></td></tr>";
                }
            }
        })
...
function details(key) {
    ...
    // fetch("http://localhost:8080/_ah/api/theatre_management/v1/theatre_management", { method: 'get' })
        ...
        .then((data) => {
            for (var i = 0; i < data.items.length; i++) {
                console.log(key.value, data.items[i].key);
                if (key.value === data.items[i].key) {
                    document.getElementById("content").innerHTML = "<form><table><tr><td>Show Name: " + data.items[i].name + " </td></tr><tr><td>Total Seats: " + data.items[i].capacity + " </td></tr><tr><td>Tickets Available: " + data.items[i].available + " </td></tr><tr><td><input id='tickets' type='number' align='center' placeholder='No. of tickets' required></td></tr><tr><td class='transparent'><button id='key' onclick='validate()' type='button' value='" + data.items[i].key + "' class='transparent'>Book</button></td></tr></table></form>";
                    break;
                }
            }
        })
...
function validate() {
    ...
    // fetch("http://localhost:8080/_ah/api/theatre_management/v1/theatre_management", { method: 'get' })
        ...
        .then((data) => {
            for (var i = 0; i < data.items.length; i++) {
                if (document.getElementById("key").value === data.items[i].key) {
                    if (tickets > data.items[i].available) {
                        ...
                        book(json_data, data.items[i].key);
                    }
                }
            }
        });
